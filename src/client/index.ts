...
// Type-only: pulls ctx.theme (the ThemeRuntime service from ui-theme).
import type {} from '@deepseek-ai/dsh-client-ui-theme/client'
import { AYAKA_TOKENS } from '../ayaka-tokens.ts'
...
/**
 * Voice lines: each entry pairs an audio file URL with the matching text.
 * Audio and text are synced — the bubble stays visible for the audio's
 * full duration. Playback is click-only: no auto-play, no timers.
 */
interface VoiceLine {
  /** Audio file URL served from /voices/. */
  audio: string
  /** Text shown in the speech bubble, synced to the audio duration. */
  text: string
}

const VOICE_LINES: readonly VoiceLine[] = [
  {
...
export function apply(ctx: ClientContext): void {
  ...
  const spawnPet = (): void => {
    ...
    const updatePetPosition = (): void => {
      ...
      let inputCard: HTMLElement | null = null
      if (textarea !== null) {
        // Walk up to find the card container (has border-radius + background)
        let el: HTMLElement | null = textarea as HTMLElement
    ...
    const speech = pet.querySelector<HTMLElement>('.ayaka-pet-speech')
    const headWrap = pet.querySelector<HTMLElement>('.ayaka-pet-head-wrap')
    let lineIndex = 0
